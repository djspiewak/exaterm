#[cfg(target_os = "linux")]
mod linux {
    use exaterm_gtk::test_support::mount_scenario;
    ...
    pub fn main() {
        ...
        run("battlefield_single_summarized", battlefield_single_summarized);
    ...
    fn battlefield_single_sparse() {
        ...
        assert_exists(
            &scene,
            &selector(&selectors::battlefield_card_terminal_slot(UiSessionKey::Shell1)),
        )
        .unwrap();
        assert_not_exists(
            &scene,
        ...
        assert_not_exists(
            &scene,
            &selector(&selectors::battlefield_card_attention_bar(UiSessionKey::Shell1)),
        )
        .unwrap();
    ...
    fn battlefield_four_mixed() {
        ...
        assert_count(&scene, &selector(&selectors::battlefield_card(UiSessionKey::Shell1)), 1)
            .unwrap();
        for card in &cards {
            assert_exists(&scene, card).unwrap();
        }
        ...
        for key in [UiSessionKey::Shell1, UiSessionKey::Shell2, UiSessionKey::Shell3, UiSessionKey::Shell4] {
            assert_exists(&scene, &selector(&selectors::battlefield_card_scrollback(key))).unwrap();
            assert_not_exists(&scene, &selector(&selectors::battlefield_card_terminal_slot(key))).unwrap();
        }
        assert_not_exists(
            &scene,
        ...
        let selected_count = [UiSessionKey::Shell1, UiSessionKey::Shell2, UiSessionKey::Shell3, UiSessionKey::Shell4]
            .iter()
            .filter(|key| {
                scene
            ...
            .count();
        assert_eq!(selected_count, 1, "exactly one selected card should be visible");
    ...
    fn focus_single_summarized() {
        ...
        assert_exists(&scene, &selector(&selectors::focus_card(UiSessionKey::Shell2))).unwrap();
        assert_exists(
            &scene,
    ...
    ) -> (glasscheck::Harness, exaterm_gtk::test_support::MountedGtkUi) {
